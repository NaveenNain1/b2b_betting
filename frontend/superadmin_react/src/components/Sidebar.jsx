...
import {
  ...
  ChevronRight, Zap, FileText, Menu, X, Globe, Activity, Lock
} from 'lucide-react';

const navItems = [
  { label: 'Dashboard', to: '/', icon: LayoutDashboard, exact: true },
  { label: 'Tenants', to: '/tenants', icon: Globe },
  { label: 'Plans', to: '/plans', icon: CreditCard },
  { label: 'Payment Gateway', to: '/payment', icon: Zap },
  {
    label: 'Logs', icon: FileText, children: [
      { label: 'Login Logs', to: '/logs/login', icon: Activity },
      { label: 'Tenant Logs', to: '/logs/tenant', icon: Users },
    ]
  },
    ...
    return (
      <div>
        <button
          ...
          className={`sidebar-link w-full justify-between ${open ? 'text-white bg-surface-hover' : ''}`}
        >
      ...
      <div className="flex items-center justify-between p-4 border-b border-surface-border">
        {!collapsed && (
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-sm font-bold text-white">B2B Betting</p>
