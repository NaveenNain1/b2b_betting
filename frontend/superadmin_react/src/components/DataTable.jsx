      ...
      <table className="w-full border-collapse">
        <thead>
          <tr className="border-b border-surface-border">
            {columns.map((col) => (
              <th
                key={col.key}
                className={`table-th ${col.sortable ? 'cursor-pointer select-none hover:text-white transition-colors' : ''}`}
        ...
        <tbody>
          {loading ? (
            ...
            <tr>
              <td colSpan={columns.length} className="table-td text-center py-12 text-gray-500">
                {emptyMessage}
              </td>
            </tr>
          ) : (
