    ...
    <div className="space-y-4 max-w-3xl">
      <div>
        <h2 className="text-xl font-bold text-white flex items-center gap-2">
          <Monitor size={20} className="text-brand-400" /> Active Sessions
        </h2>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3 mt-1">
                    <span className="text-xs text-gray-500 flex items-center gap-1">
                      <MapPin size={11} /> {s.ip_address || 'Unknown IP'}
                    </span>
                    <span className="text-xs text-gray-500 flex items-center gap-1">
                      <Clock size={11} /> {new Date(s.createdAt).toLocaleString()}
                    </span>
                  </div>
                  <p className="text-xs text-gray-600 mt-1 truncate max-w-xs">{s.user_agent}</p>
                </div>
              </div>
              {idx !== 0 && (
                <button
      ...
      <ConfirmDialog
        ...
        title="Revoke Session"
        message="This will immediately log out that device. Continue?"
        confirmLabel="Revoke"
